...
Private Sub Preparer_Hub_Central()
    ...
    btnSet.Fill.ForeColor.RGB = RGB(40, 70, 180) '(128, 128, 128) ' Gris Pro Neutre
    ...
    btnSync.Fill.ForeColor.RGB = RGB(40, 70, 180) '(128, 128, 128) 'RGB(250, 218, 94) JAUNE 'RGB(46, 204, 113) ' Vert Émeraude
...
End Function
' =========================================================================
' GATEKEEPER 0 : PROTECTION LOGICIELLE NATIVE (100% MULTILINGUE)
' =========================================================================
Public Sub VERIFIER_LICENCE()
    Dim wsSys As Worksheet: Set wsSys = ThisWorkbook.Sheets("SYS_Config")
    Dim tblConf As ListObject
    On Error Resume Next: Set tblConf = wsSys.ListObjects("T_SYS_Config"): On Error GoTo 0
    If tblConf Is Nothing Then Exit Sub
    ...
    ' 1. Lecture de la clé
    Dim licKey As String: licKey = ""
    Dim i As Long
    For i = 1 To tblConf.ListRows.Count
        If tblConf.DataBodyRange(i, 1).Value = "SYS_LICENCE_KEY" Then licKey = CStr(tblConf.DataBodyRange(i, 2).Value): Exit For
    Next i

    ' 2. Lecture de l'Empreinte Matérielle
    Dim machineID As String: machineID = GetMachineID()
    Dim expectedKey As String: expectedKey = CalculateValidKey(machineID)

    ' Validation silencieuse si le logiciel est déjà activé
    If licKey = expectedKey Then Exit Sub

    ' --- LOCALISATION SYSTÈME (Indépendant de la Base de Données) ---
    Dim sysLang As String: sysLang = GET_SYSTEM_LANGUAGE()
    Dim tTitle As String, tTitleErr As String, tTitleSec As String
    Dim tNotAct As String, tId As String, tEnter As String
    Dim tClose As String, tInvalid As String, tSuccess As String

    Select Case sysLang
        Case "FR"
            tTitle = "SFP SECURITY LOCK": tTitleErr = "ACCÈS REFUSÉ": tTitleSec = "SÉCURITÉ SFP"
            tNotAct = "LOGICIEL NON ACTIVÉ": tId = "Votre ID Machine :": tEnter = "Saisissez votre Clé d'Activation :"
            tClose = "L'activation est obligatoire pour accéder au système." & vbCrLf & "Voulez-vous fermer l'application ?"
            tInvalid = "CLÉ INVALIDE." & vbCrLf & "Veuillez vérifier votre saisie et réessayer."
            tSuccess = "LICENCE VALIDÉE. Merci de votre achat."
        Case "ES"
            tTitle = "SFP SECURITY LOCK": tTitleErr = "ACCESO DENEGADO": tTitleSec = "SEGURIDAD SFP"
            tNotAct = "SOFTWARE NO ACTIVADO": tId = "Su ID de Máquina :": tEnter = "Ingrese su Clave de Activación :"
            tClose = "La activación es obligatoria para acceder al sistema." & vbCrLf & "¿Desea cerrar la aplicación?"
            tInvalid = "CLAVE INVÁLIDA." & vbCrLf & "Verifique su entrada e intente nuevamente."
            tSuccess = "LICENCIA VALIDADA. Gracias por su compra."
        Case "PT"
            tTitle = "SFP SECURITY LOCK": tTitleErr = "ACESSO NEGADO": tTitleSec = "SEGURANÇA SFP"
            tNotAct = "SOFTWARE NÃO ATIVADO": tId = "Seu ID de Máquina :": tEnter = "Insira sua Chave de Ativação :"
            tClose = "A ativação é obrigatória para acessar o sistema." & vbCrLf & "Deseja fechar o aplicativo?"
            tInvalid = "CHAVE INVÁLIDA." & vbCrLf & "Verifique sua entrada e tente novamente."
            tSuccess = "LICENÇA VALIDADA. Obrigado pela sua compra."
        Case "DE"
            tTitle = "SFP SECURITY LOCK": tTitleErr = "ZUGRIFF VERWEIGERT": tTitleSec = "SFP SICHERHEIT"
            tNotAct = "SOFTWARE NICHT AKTIVIERT": tId = "Ihre Maschinen-ID :": tEnter = "Geben Sie Ihren Aktivierungsschlüssel ein :"
            tClose = "Die Aktivierung ist obligatorisch, um auf das System zuzugreifen." & vbCrLf & "Möchten Sie die Anwendung schließen?"
            tInvalid = "UNGÜLTIGER SCHLÜSSEL." & vbCrLf & "Bitte überprüfen Sie Ihre Eingabe und versuchen Sie es erneut."
            tSuccess = "LIZENZ VALIDIERT. Vielen Dank für Ihren Kauf."
        Case "IT"
            tTitle = "SFP SECURITY LOCK": tTitleErr = "ACCESSO NEGATO": tTitleSec = "SICUREZZA SFP"
            tNotAct = "SOFTWARE NON ATTIVATO": tId = "Il tuo ID Macchina :": tEnter = "Inserisci la tua Chiave di Attivazione :"
            tClose = "L'attivazione è obbligatoria per accedere al sistema." & vbCrLf & "Vuoi chiudere l'applicazione?"
            tInvalid = "CHIAVE NON VALIDA." & vbCrLf & "Controlla il tuo inserimento e riprova."
            tSuccess = "LICENZA CONVALIDATA. Grazie per l'acquisto."
        Case "NL"
            tTitle = "SFP SECURITY LOCK": tTitleErr = "TOEGANG GEWEIGERD": tTitleSec = "SFP BEVEILIGING"
            tNotAct = "SOFTWARE NIET GEACTIVEERD": tId = "Uw Machine-ID :": tEnter = "Voer uw Activeringssleutel in :"
            tClose = "Activering is verplicht om toegang te krijgen tot het systeem." & vbCrLf & "Wilt u de applicatie sluiten?"
            tInvalid = "ONGELDIGE SLEUTEL." & vbCrLf & "Controleer uw invoer en probeer het opnieuw."
            tSuccess = "LICENTIE GEVALIDEERD. Bedankt voor uw aankoop."
        Case "SV"
            tTitle = "SFP SECURITY LOCK": tTitleErr = "ÅTKOMST NEKAD": tTitleSec = "SFP SÄKERHET"
            tNotAct = "PROGRAMVARA EJ AKTIVERAD": tId = "Ditt Maskin-ID :": tEnter = "Ange din Aktiveringsnyckel :"
            tClose = "Aktivering är obligatorisk för att komma åt systemet." & vbCrLf & "Vill du stänga applikationen?"
            tInvalid = "OGILTIG NYCKEL." & vbCrLf & "Kontrollera din inmatning och försök igen."
            tSuccess = "LICENS VALIDERA. Tack för ditt köp."
        Case Else ' EN (Fallback International)
            tTitle = "SFP SECURITY LOCK": tTitleErr = "ACCESS DENIED": tTitleSec = "SFP SECURITY"
            tNotAct = "SOFTWARE NOT ACTIVATED": tId = "Your Machine ID :": tEnter = "Enter your Activation Key :"
            tClose = "Activation is mandatory to access the system." & vbCrLf & "Do you want to close the application?"
            tInvalid = "INVALID KEY." & vbCrLf & "Please check your entry and try again."
            tSuccess = "LICENSE VALIDATED. Thank you for your purchase."
    End Select

    ' 3. BLOCAGE DE SÉCURITÉ COMMERCIAL (BOUCLE INFINIE)
    Dim userKey As String
    Dim isUnlocked As Boolean: isUnlocked = False
    
    Do While Not isUnlocked
        userKey = InputBox(tNotAct & vbCrLf & vbCrLf & _
                    tId & " " & machineID & vbCrLf & vbCrLf & _
                    tEnter, tTitle)

        If UCase(Trim(userKey)) = expectedKey Then
            isUnlocked = True
        ElseIf userKey = "" Then
            Dim rep As VbMsgBoxResult
            rep = MsgBox(tClose, vbYesNo + vbQuestion, tTitleSec)
            If rep = vbYes Then
                ThisWorkbook.Close SaveChanges:=False
                Exit Sub
            End If
        Else
            MsgBox tInvalid, vbExclamation, tTitleErr
        End If
    Loop

    ' 4. SAUVEGARDE DE LA LICENCE
    wsSys.Unprotect "SFP_ADMIN_2026"
    Dim found As Boolean: found = False
    For i = 1 To tblConf.ListRows.Count
        If tblConf.DataBodyRange(i, 1).Value = "SYS_LICENCE_KEY" Then
            tblConf.DataBodyRange(i, 2).Value = expectedKey
            found = True: Exit For
        End If
    Next i
    If Not found Then
        Dim nr As ListRow: Set nr = tblConf.ListRows.Add
        nr.Range(1, 1).Value = "SYS_LICENCE_KEY": nr.Range(1, 2).Value = expectedKey: nr.Range(1, 3).Value = "SFP Pro License"
    End If
    wsSys.Protect "SFP_ADMIN_2026", UserInterfaceOnly:=True
    
    MsgBox tSuccess, vbInformation, "SFP UNLOCKED"
End Sub
' --- FIN PATCH ---

' --- Lecture via Variables d'Environnement (Anti-Overflow) ---
Private Function GetMachineID() As String
    Dim sysName As String: sysName = Environ("COMPUTERNAME")
    Dim usrName As String: usrName = Environ("USERNAME")
    If sysName = "" Then sysName = "HOST"
    If usrName = "" Then usrName = "USER"
    
    Dim combined As String: combined = UCase(sysName & usrName)
    Dim i As Integer, hash As Long: hash = 0
    For i = 1 To Len(combined)
        hash = hash + (CLng(Asc(Mid(combined, i, 1))) * CLng(i))
    Next i
    GetMachineID = UCase(Left(sysName, 4)) & "-" & Right("0000" & UCase(Hex(hash)), 4)
End Function

' --- Algorithme de Cryptage Embarqué (Anti-Overflow) ---
